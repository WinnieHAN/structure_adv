...
def zh_gpt(cands, refs):
    ...
    old_state_dict = torch.load("./pretrained_model_gpt/model_state_epoch_62.th",
    ...
    with torch.no_grad():
        for step, s in enumerate(cands):  # actually here is a batch with batchsize=1
            ...
            ppls.append(11.5 - log_ppl.cpu().item())
    np.savetxt(SCORE_PREFIX + 'temp_ppl.txt', np.array(ppls))


def zh_gpt2(cands, refs):
    import transformers
    from tokenizations import tokenization_bert
    full_tokenizer = tokenization_bert.BertTokenizer(vocab_file='/home/zhanglw/code/GPT2-Chinese/pretrained/vocab.txt')
    model = transformers.modeling_gpt2.GPT2LMHeadModel.from_pretrained('/home/zhanglw/code/GPT2-Chinese/pretrained')
    model.eval()
    model.to(device)
    new_cands = []
    for cand in cands:
        cand = full_tokenizer.convert_tokens_to_ids(full_tokenizer.tokenize(cand))
        new_cands.append([full_tokenizer.convert_tokens_to_ids('[MASK]')] + cand + [full_tokenizer.convert_tokens_to_ids('[CLS]')])
    ppls = []
    with torch.no_grad():
