...
def main():
    ...
    args_parser.add_argument('--seq2seq_load_path', default='models/seq2seq/seq2seq_save_model', type=str,
                             help='seq2seq_load_path')
    args_parser.add_argument('--network_load_path', default='models/seq2seq/network_save_model', type=str,
    ...
    for i in range(EPOCHS):
        ...
        testi = 0
        for batch in conllx_data.iterate_batch_tensor(data_dev,
                                                      batch_size):  # for _ in range(1, num_batches + 1):  word, char, pos, heads, types, masks, lengths = conllx_data.get_batch_tensor(data_dev, batch_size, unk_replace=unk_replace)  # word:(32,50)  char:(32,50,35)
            word, char, pos, heads, types, masks, lengths = batch
            inp = word
            # inp, _ = seq2seq.add_noise(word, lengths)
            dec_out = word
