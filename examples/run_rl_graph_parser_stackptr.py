...
from neuronlp2.models import BiRecurrentConvBiAffine, StackPtrNet
...
def main():
    ...
    args_parser.add_argument('--objective', choices=['biaffine', 'stackptr'], default='biaffine', help='objective model.')
    ...
    arg_path = "./models/parsing/stack_ptr/network.pt.arg.json"
    [word_dim, num_words, char_dim, num_chars, pos_dim, num_pos, num_filters, window, mode, input_size_decoder,
     hidden_size, encoder_layers, decoder_layers, num_types, arc_space, type_space] = \
    ...
    if args.treebank == 'ptb':
        if obj == "biaffine":
            ...
            network.load_state_dict(torch.load('models/parsing/stack_ptr/network.pt'))
    ...
    if parser_select[0] == 'stackPtr':
        ...
        biaffine_parser.load_state_dict(torch.load('models/parsing/biaffine/network.pt'))
    ...
    for epoch_i in range(0, num_epochs):
        ...
        for kkk in range(1, num_batches + 1):  # range(1, 1):
            ...
            if True:  # inp.size()[1]<15:#True:  #inp.size()[1]<15:
                ...
                with torch.no_grad():
                    try:
                        heads_pred, types_pred, _, _ = network.decode(sel1, input_char=None, input_pos=None, mask=masks_sel,
                                                                      length=lengths_sel,
